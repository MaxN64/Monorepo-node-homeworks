...
function authenticateJWT(req, res, next) {
  ...
  if (!authHeader) {
    return res
      .status(401)
      .json({ error: "Токен не найден в заголовке Authorization" });
  }
  const token = authHeader.split(" ")[1];
  if (!token) {
    return res
      .status(401)
      .json({ error: "Некорректный формат заголовка Authorization" });
  }
...
app.post("/login", async (req, res) => {
  ...
  if (!username || !password) {
    return res
      .status(400)
      .json({ error: "Укажите username и password в теле запроса" });
  }
...
app.get("/", (req, res) => {
  res.send("Server is up. Use /login and /update-email endpoints.");
});
