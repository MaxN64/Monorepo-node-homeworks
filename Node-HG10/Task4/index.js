...
app.delete('/delete-account', authenticateJWT, (req, res) => {
  ...
  return res.json({
    ...
    user: { id: deleted.id, username: deleted.username, email: deleted.email, role: deleted.role },
  });
});
...
app.put('/update-role', authenticateJWT, authorizeRole('admin'), (req, res) => {
  ...
  if (!userId || !role) {
    return res.status(400).json({ error: 'Поле userId и role обязательны' });
  }
  if (!['user', 'admin'].includes(role)) {
    return res.status(400).json({ error: 'Недопустимая роль. Используйте: user, admin' });
  }
...
app.get('/', (req, res) => {
  res.send('Server is up. Use /login, /update-email, /delete-account, /update-role, /refresh-token endpoints.');
});
